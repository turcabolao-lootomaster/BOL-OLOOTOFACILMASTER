        ...
        {showSuccessModal && (
          ...
          >
            <motion.div 
              ...
              animate={{ scale: 1, y: 0 }}
              className="bg-white border-2 border-lotofacil-purple/30 rounded-3xl w-full max-w-md p-6 sm:p-8 space-y-6 shadow-[0_0_50px_rgba(107,33,168,0.3)] text-center relative overflow-hidden"
            >
              <div className="absolute top-0 left-0 w-full h-2 bg-gradient-to-r from-lotofacil-purple via-lotofacil-pink to-lotofacil-purple" />
              ...
              <div className="flex flex-col items-center gap-4">
                ...
                <div>
                  <h3 className="text-2xl font-display tracking-widest uppercase text-slate-900 leading-tight">Apostas Realizadas!</h3>
                  <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-1">Concurso #{activeContest?.number}</p>
                </div>
              </div>

              <div className="bg-slate-50 border border-slate-100 rounded-2xl p-6 space-y-4">
                <div className="flex items-center justify-between border-b border-slate-200 pb-3">
                  <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Qtd Apostas</span>
                  <span className="text-xl font-bold text-slate-900">{lastBetIds.length}</span>
                </div>
                <div className="flex items-center justify-between py-1">
                  <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Total a Pagar</span>
                  <span className="text-2xl font-black text-lotofacil-purple">R$ {(lastBetIds.length * 10).toFixed(2).replace('.', ',')}</span>
                </div>
                
                {sellerPixKey && (
                  <div className="pt-3 space-y-2">
                    <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Chave PIX do Vendedor</p>
                    <button 
                      onClick={() => {
                        navigator.clipboard.writeText(sellerPixKey);
                        setFeedback({ message: 'Chave PIX copiada!', type: 'success' });
                      }}
                      className="w-full bg-slate-100 hover:bg-slate-200 py-3 rounded-xl font-mono font-bold text-slate-700 transition-all border border-slate-200 group active:scale-95 flex items-center justify-center gap-2"
                    >
                      {sellerPixKey}
                      <Sparkles size={14} className="text-lotofacil-yellow opacity-0 group-hover:opacity-100 transition-opacity" />
                    </button>
                    <p className="text-[8px] text-slate-400 italic">Toque para copiar a chave</p>
                  </div>
                )}
              </div>
              ...
              <div className="space-y-3 pt-2">
                <button 
                  ...
                  className="w-full bg-[#25D366] hover:bg-[#128C7E] text-white py-4 rounded-2xl flex items-center justify-center gap-3 font-bold uppercase tracking-widest text-xs transition-all shadow-[0_10px_30px_rgba(37,211,102,0.4)] active:scale-95 animate-bounce-subtle"
                >
          ...
          <div className="space-y-1.5 px-1">
            ...
            <div className="relative">
              <input 
                ...
                required
                className="w-full bg-white border-2 border-slate-200 rounded-xl py-3 px-4 focus:outline-none focus:border-lotofacil-purple transition-all text-xs sm:text-sm font-bold text-slate-900 placeholder:text-slate-300 uppercase shadow-sm"
              />
            </div>
          </div>

          <div className="grid grid-cols-5 gap-1 w-full sm:px-0 px-1">
            {Array.from({ length: 25 }, (_, i) => i + 1).map(num => (
              <motion.button
                ...
                className={cn(
                  "aspect-square rounded-lg sm:rounded-xl transition-all flex items-center justify-center border-2 text-xs sm:text-lg font-bold shadow-sm",
                  selectedNumbers.includes(num)
                    ? "bg-lotofacil-purple border-lotofacil-purple text-white shadow-[0_0_15px_rgba(107,33,168,0.4)]"
                    : "bg-white border-slate-200 text-slate-400 hover:border-lotofacil-purple/30"
                )}
              >
