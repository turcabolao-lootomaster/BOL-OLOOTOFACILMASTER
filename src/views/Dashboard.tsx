  ...
  return (
    <div className={cn(
      ...
      {(() => {
        ...
        const lastDrawWithResults = [...activeContest.draws].reverse().find(d => d.results && d.results.length > 0);
        if (!lastDrawWithResults) return null;
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 relative z-10">
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-[10px] sm:text-xs font-black uppercase tracking-[0.2em] text-lotofacil-purple">
                    ÚLTIMO RESULTADO (SORTEIO #{lastDrawWithResults.number}) <span className="text-white/40 ml-1">#{activeContest.number}</span>
                  </h2>
                </div>
                <p className="text-[8px] sm:text-[10px] text-white/40 uppercase tracking-widest mt-1">Confira os números do último sorteio realizado</p>
              </div>
              <div className="flex flex-wrap gap-1.5 sm:gap-2">
                {[...lastDrawWithResults.results].sort((a, b) => a - b).map((num, i) => (
                  <motion.span 
                    key={num} 
                    initial={{ scale: 0 }}
                    animate={{ scale: 1 }}
