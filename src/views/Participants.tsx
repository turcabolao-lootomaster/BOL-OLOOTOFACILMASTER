...
import { Trophy, Medal, Search, Share2, DollarSign, TrendingUp, Info, Zap, Download, X, Lock, ChevronRight, FileText } from 'lucide-react';
...
const Participants: React.FC = () => {
  ...
  const handleDownloadPDF = () => {
    ...
    doc.text(`LISTA DE PARTICIPANTES - CONCURSO #${contest?.number || '...'}`, 148.5, 15, { align: 'center' });

    // Prize Info
    doc.setFontSize(10);
    doc.setTextColor(30, 41, 59);
    doc.text('DETALHES DA PREMIAÇÃO', 14, 35);
    doc.setFontSize(8);
    doc.text(`Rapidinha: ${prizes.rapidinha.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}`, 14, 42);
    doc.text(`Campeão: ${prizes.champion.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}`, 14, 47);
    ...
    doc.save(`Participantes_Bolao_Concurso_${contest?.number || '...'}.pdf`);
  };
                    ...
                    >
                      ...
                      <td className="px-1 sm:px-6 py-4 text-center">
                        <div className={cn(
                          ...
                          idx < 3 ? "bg-white border-lotofacil-purple/30" : "bg-slate-50 border-slate-200"
                        )}>
                          <span className={cn(
                            "text-xs sm:text-lg font-display tracking-tighter leading-none",
                            isExpanded ? "text-white" : total >= 27 ? "text-white" : "text-lotofacil-purple"
                          )}>
                            {total.toString().padStart(2, '0')}
                          </span>
                          <span className={cn(
                            "text-[5px] sm:text-[7px] font-black uppercase tracking-tighter mt-0.5",
                            isExpanded ? "text-white/40" : "text-slate-400"
                          )}>
                            PTS
                          </span>
                        </div>
